...
with mx_order:
    curr_orders = data.loc[data['order_year']==CURR_YEAR, 'order_id'].values[0]
    prev_orders = data.loc[data['order_year']==PREV_YEAR, 'order_id'].values[0]
    order_diff_pct = 100.0 * (curr_orders - prev_orders) / prev_sales
    st.metric("Number of Orders", value=format_big_number(curr_orders), delta=f'{order_diff_pct:.2f}%')

with mx_customer:
    curr_customers = data.loc[data['order_year']==CURR_YEAR, 'customer_id'].values[0]
    prev_customers = data.loc[data['order_year']==PREV_YEAR, 'customer_id'].values[0]
    customer_diff_pct = 100.0 * (curr_customers - prev_customers) / prev_sales
    st.metric("Number of Customers", value=curr_customers, delta=f'{customer_diff_pct:.2f}%')
...
with mx_profit_pct:
    ...
    st.metric("Profit Percentage", value=f'{curr_profit_pct:.2f}%', delta=f'{profit_diff_pct:.2f}%')
